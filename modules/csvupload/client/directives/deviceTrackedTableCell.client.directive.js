(function() {
  angular.module("csvupload").directive("deviceTrackedTableCell", deviceTrackedTableCell);

  deviceTrackedTableCell.$inject = [];

  function deviceTrackedTableCell() {
    return {
      restrict: "A",
      priority: -1,
      scope: true,
      require: ["^deviceTrackedTableRow", "ngForm"],
      controller: deviceTrackedTableCellController
    };
  }

  deviceTrackedTableCellController.$inject = ["$attrs", "$element", "$scope"];

  function deviceTrackedTableCellController($attrs, $element, $scope) {
    var self = this;
    var cellFormCtrl = $element.controller("form");
    var cellName = cellFormCtrl.$name;
    var trackedTableRowCtrl = $element.controller("deviceTrackedTableRow");
